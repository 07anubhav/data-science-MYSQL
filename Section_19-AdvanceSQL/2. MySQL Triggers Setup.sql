/*
Triggers: It is a stored program, associated with a table that will be activated automatically once a specific event occurs.
	The event must be related to the associated table and the represented by one of the following 3 DML statements:
	1. INSERT
    2. UPDATE
    3. DELETE

A trigger is a MySQL object that can 'trigger' a specific action or calculation 'before' or 'after' an INSERT, UPDATE or DELETE has been executed.

System-functions: These are built in functions, often applied in practice, they provide data about the moment of the execution of a certain query
*/
...
# Now, let’s look at a BEFORE UPDATE trigger. The code is similar to the one of the trigger we created above, with two substantial differences.
/*
BEFORE UPDATE
*/
DELIMITER $$
